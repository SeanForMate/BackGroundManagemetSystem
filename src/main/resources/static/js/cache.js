...
layui.use(['form','jquery',"layer"],function() {
    ...
    function lockPage(){
        layer.open({
            title : false,
            type : 1,
            content : '<div class="admin-header-lock" id="lock-box" style="background:#F2F2F2;">'+
                            '<div class="admin-header-lock-img"><img src="static/images/face.jpg" class="userAvatar"/></div>'+
                            '<div class="admin-header-lock-name" id="lockUserName">'+window.localStorage.getItem('realName')+'</div>'+
                            '<div class="input_btn">'+
    ...
    $(".larger").click(function(){
    	...
    	if(largerFlag==0){
            ...
            // 切换成退出全屏图标
            $("#largerFull").removeClass("layui-icon-screen-full");
			$("#largerFull").addClass("layui-icon-screen-restore");
			$("#largerFull").parent().find("cite").text("退出");
			largerFlag = 1;
    	}else{
    		...
    		$("#largerFull").removeClass("layui-icon-screen-restore");
			$("#largerFull").addClass("layui-icon-screen-full");
			$("#largerFull").parent().find("cite").text("全屏");
			largerFlag = 0;
    	}
    ...
    $(".signOut").click(function(){
		layer.confirm("是否确认退出", {icon: 3, title: "提醒"}, function (index) {		
