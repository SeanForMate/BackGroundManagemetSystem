...
layui.define(["element","jquery"],function(exports){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                    	...
                    	ulHtml += '<i class="ok-icon" data-icon="'+data[i].icon+'">'+data[i].icon+'</i>';
                    }
                }
                if(data[i].icon != undefined && data[i].icon != ''){
                    ulHtml += '<i class="ok-icon" data-icon="'+data[i].icon+'">'+data[i].icon+'</i>';
                ...
                for(var j=0;j<data[i].children.length;j++){
                    ...
                    if(data[i].children[j].icon != undefined && data[i].children[j].icon != ''){
                        if(data[i].children[j].icon.indexOf("icon-") != -1){
                        	...
                        	ulHtml += '<i class="ok-icon" data-icon="'+data[i].children[j].icon+'">'+data[i].children[j].icon+'</i>';
                        }
                    }
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="/main"><i class="ok-icon" data-icon="&#xe66f;">&#xe66f;</i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			var title = '';
			if(_this.find("i.seraph,i.layui-icon,i.ok-icon").attr("data-icon") != undefined){
				...
				}if(_this.find("i.ok-icon").attr("data-icon") != undefined){
					title += '<i class="ok-icon">'+_this.find("i.ok-icon").attr("data-icon")+'</i>';
				}
			}
			//已打开的窗口中不存在
